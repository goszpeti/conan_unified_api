...
def test_get_profiles(conan_api: ConanUnifiedApi):
    profiles = conan_api.get_profiles()
    assert len(profiles) >= 1
    assert "default" in profiles


def test_get_profiles_path(conan_api: ConanUnifiedApi):
    profiles_path = conan_api.get_profiles_path()
    assert (profiles_path / "default").is_file()


def test_get_profile_settings(conan_api: ConanUnifiedApi):
    settings = conan_api.get_profile_settings("default")
    assert settings.get("os") == platform.system()
    assert settings.get("arch") == "x86_64"


def test_get_default_settings(conan_api: ConanUnifiedApi):
    settings = conan_api.get_default_settings()
    assert settings.get("os") == platform.system()
    assert settings.get("arch") == "x86_64"


def test_conan_profile_name_alias_builder(conan_api: ConanUnifiedApi):
