...
import conan_unified_api  # for mocker
import psutil
from conan_unified_api import INVALID_PATH, PKG_NAME
from conan_unified_api.app.system import (calc_paste_same_dir_name,
...
def test_choose_run_file(tmp_path, mocker):
    ...
    """
    # Mock away the calls
    mocker.patch('conan_unified_api.app.system.open_file')
    mocker.patch('conan_unified_api.app.system.execute_app')

    # test with nonexistant path - nothing should happen (no exception raising)

    run_file(Path(INVALID_PATH), False, "")
    conan_unified_api.app.system.open_file.assert_not_called()
    conan_unified_api.app.system.execute_app.assert_not_called()
    ...
    conan_unified_api.app.system.open_file.assert_called_once_with(test_file)
...
def test_choose_run_script(tmp_path, mocker):
    ...
    conan_unified_api.app.system.execute_app.assert_called_once_with(test_file, False, "")
